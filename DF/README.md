...
[专题方案与逐层分工](../HBM/address-interleaving-plan.md) · [本模块具体落点](research-plan.md#u24请求地址到-hbm-bank-的跨模块落点)。重点解释 system/local/remote 选择、交织粒度、hash/适用的非二次幂映射、CS/内存目标及下游局部地址；将 stack/channel/UMC 实例关系逐项核对。ATL 仅作配置与逆向解码方法参考，不当作在线硬件流水。 先读取现有资料索引，专题例子跨模块共用，实际连接与参数保留证据边界。
