...
资料集入口：[资料集与来源总入口](../sources.md)，当前公开研究先查看 P6 来源组。模块上下文入口见 [SWITCH/README.md](../SWITCH/README.md)的“资料集与接续”；先浏览资料简介、阅读定位与适用边界，再针对本轮问题回到原文。后续独立建立模块资料集时同步更新这两个入口，内容简介集中维护。

下一轮开始时先读本页、主文档第 49 章及当前代码；从 GitHub 当前提交检查是否有并行修改。需要补充结论时区分公开来源、参考设计与目标芯片事实，新增细节同时审查是否改变 buffer、credit、ownership、ordering 或完成点。
