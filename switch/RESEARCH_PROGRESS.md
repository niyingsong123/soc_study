...
用户最新指示：暂不重新检查六轮安排，先按已经确认的写法重写现有中文详细稿，包含上下游模块及其接口。本节记录可核实的完成点，后续完成一个可复核阶段后原位更新。

- 最后完成：通用写法已保存为范本 v1.5，上下游及接口要求已补入范本、项目上下文和协作约定；对应提交为 `c236fc512472e392a22a232a2846f776f316e398`。
- 本次状态核对：GitHub `main` 仍指向上述提交；详细稿最近一次文件提交为 `585661dfa3d90f3d0488cd3f6c5d50f6be8103a6`。尚无本次重写的正文提交，不能把已保存的方法当成重写完成。
- 当前未完成：按新顺序改写正文、核查关键疑点、技术与阅读理解审核，以及受影响章节链接/入口同步。原第 1、2 轮已完成，第 3–6 轮未完成。
- 下一步：读取现有详细稿与本模块资料索引，按“需求→功能/feature→上下游和整体微架构→子模块与接口→完整过程→核心机制及复杂行为”原位重组；复用有效结论，关键疑点定向查证，非必要深度保留可选问题。六轮安排本次不另行复查。
- 异常接续：保留已完成内容与证据，并在本节写清最后完成步骤、尚未完成内容和下一项动作，不把任务未继续执行描述成后台持续运行。
