...
模块索引告诉你“这篇讲什么、何时读”；逐篇笔记保存技术细节、限制和位置；研究方案保存微架构问题及轮次。先读模块上下文/结构，再查索引和相关笔记，按需回原文。[维护范本 v1.5](chip-study-plan.md) · [项目上下文](project-context.md) · [研究路线图](research-roadmap.md)。

**使用状态：** 已完成笔记/索引建设及本次可访问范围的补读；相关原版规范、外部 SDMA 与目标 CF 证据仍有具体缺口；HBM 器件手册按 U23 降为可选参考，不用来源数替代技术验收。
...
- **U26（2026-09-25）：** 交流使用中文；新建的研究笔记、中间分析和接续结果默认英文，用户明确要求阅读/展示的中文文档使用中文。既有中文内容不批量翻译，当前 SWITCH 详细稿按中文读者稿重整。执行规则见[范本](chip-study-plan.md#文档用途与单模块聚焦)。
- **U27（2026-09-25）：** 研究时一次聚焦一个模块，只补入必要的相邻接口上下文；结论和具体跨模块问题保存在该模块。相关模块的必要详细结论形成后再集中研究完整联系，接口正确性疑点当下处理；保留 U24 的 HBM 核心专题。切换模块选择性恢复上下文，不声称可清除已有会话信息。
- **U28（2026-09-25）：** 用户要求仔细重整 SWITCH 详细稿：先说明为什么需要模块，再分析典型功能/feature、子模块划分及接口，后续细化以帮助人理解为目标；必要疑点网上查证。对不影响子模块、feature、数据流、控制流、接口等理解的过深细节，就近留问题供用户按需追问。用户进一步要求把商定写法保存到项目上下文供其他模块复用。依据这一要求保存的[详细文档写作方法](chip-study-plan.md#详细文档写作方法)强调完整微架构、因果解释、贯穿例子、正文与资料笔记分工及技术/读者审核；可选深入问题不计为未完成任务，关键未知仍需核实。本次保存方法不代表已重写正文或完成新论文轮次。

- **U29（2026-09-25）：** 用户进一步强调整体微架构必须考虑上下游模块及其接口。相应图文应说明外部端点、本模块边界、内部接入位置和请求/数据/返回路径，并解释接口约定如何影响子模块与 feature；这是当前模块的基础研究，不能因逐模块聚焦而推迟到完整跨模块专题。具体写法见[范本](chip-study-plan.md#整体微架构包含上下游模块及其接口)。
