...
每个学习模块现在都有 `sources/README.md` 与逐篇技术笔记，统一入口见 [sources.md](sources.md)。`sources/` 表示资料组织，不新增芯片子模块或改变本表的硬件关系。C01–C05 已依据现有页图形成参考笔记，目标映射、名称冲突和算例纠错见对应笔记；不能仅因阅读更深入便把待确认连接改为已证实。

## HBM 地址路径研究入口（U24）

[从请求地址到 HBM bank](HBM/address-interleaving-plan.md)负责逐场景核对源端、翻译、缓存、hub/EA、DF/CS、SWITCH、UMC、PHY 与 HBM 的关系。分别记录硬件连接、地址转换/目标选择、逻辑资源映射与返回关联；只把实际参与的模块接入该路径。本次建立研究分工，不修改未经证实的 RTL 父子关系或将各代实例拼成统一拓扑。
