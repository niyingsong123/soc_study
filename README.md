...
多轮论文研究统一采用 [芯片学习与微架构研究计划](chip-study-plan.md)。本项目依据代表性数据／请求路径，先上游、后下游逐模块完成方案；控制与管理主题按事件及依赖安排。SWITCH 已完成的两轮作为既有基础，结合路径依赖安排原计划复核；详细展开由后续本地 Codex 接续。项目范围与实际进度见 [项目上下文](project-context.md)。
