...
多轮论文研究统一采用 [芯片学习与微架构研究计划](chip-study-plan.md)。本项目已按代表性数据／请求路径和控制事件依赖形成 [模块方案及接续顺序](research-roadmap.md)。当前用户指定先重整并审核 SWITCH 既有详细稿；本次已保存写法与接续方案，正文重整尚未执行，不增加已完成轮次。其他模块按需要复用已有笔记，必要接口先核对，整体保持先上游、后下游；未指定模块且明确启动新模块论文时才默认 GC 第 1 轮。SWITCH 后续以 [修订方案](SWITCH/research-plan.md) 为准。项目范围与实际进度见 [项目上下文](project-context.md)。
