...
- [芯片学习与微架构研究计划](chip-study-plan.md)：当前 v1.5，包含微架构规划、资料集及引用、单模块聚焦、语言规则与[详细文档写作方法](chip-study-plan.md#详细文档写作方法)，供后续各模块复用。
- [项目上下文](project-context.md)：当前 AMD 项目范围、研究顺序、SWITCH 进度、资料现状与后续问题。
- [模块关系与完整映射](module-map.md)：包含关系、接口关系、归档位置及待确认项。
- [资料集与来源](sources.md)：本地 SDMA 只读参考与公开架构资料的总入口；现有 103 篇来源笔记与 19 个模块索引，每篇附内容导读、细节、版本和实际阅读范围。
- [SDMA 外部项目入口](SDMA/README.md)：FE、BE、TBE 的详细研究继续在独立项目维护。

## 当前研究规划

已完成各模块的多轮规划及资料索引，逐篇笔记为阶段性成果：[研究路线图和全部方案](research-roadmap.md)。18 个模块按 3–6 轮安排，SDMA 另有仅限系统接口的接续方案；详细论文由后续本地 Codex 逐轮补充。SWITCH 已完成第 1–3 轮，第 4–6 轮待执行；最新内容与进度见文末同步入口。本批资料扩充与方案修订已保存，但详细阅读与笔记完善尚未全部完成；IO4 与 L1–L3 共 4 篇尚无本次可读正文；MEM11/MEM16 的规范转录选读等仍有原版/深度缺口；HBM 器件数据表按用户 U23 调整为按需参考，不再列作必补项。接续先查[资料任务检查与待补项](source-reading-audit.md)，再读模块索引和相关笔记。

当前明确的跨模块主线：[从请求地址到 HBM bank](HBM/address-interleaving-plan.md)。研究资源划分、逐层地址映射/interleave、请求拆分、返回和性能影响；方案已保存；先完成各模块必要的独立研究、核实接口并保存依赖问题，相关基础具备后再集中统一完整路径、地址演算与返回。厂商数据表可选不影响这些核心机制的研究深度。
...
多轮论文研究统一采用 [芯片学习与微架构研究计划](chip-study-plan.md)。本项目已按代表性数据／请求路径和控制事件依赖形成 [模块方案及接续顺序](research-roadmap.md)。SWITCH [中文详细稿 v2.3](SWITCH/switch_detailed_guide.md)已同步本地第三轮 NI 修订，核查范围见[第三轮记录](SWITCH/round3-review.md)；第 1–3 轮完成，第 4–6 轮待执行。其他模块按需要复用已有笔记，必要接口先核对，整体保持先上游、后下游；未指定模块且明确启动新模块论文时才默认 GC 第 1 轮。SWITCH 后续以 [修订方案](SWITCH/research-plan.md) 为准。项目范围与实际进度见 [项目上下文](project-context.md)。

详细稿先讲需求与典型功能，再讲整体微架构、子模块、接口和完整过程，随后展开核心机制与复杂行为；不影响理解的深层细节留作就近“可选深入问题”，不算未完成任务。影响正确性的疑点须核实或明确条件。完整方法见[范本](chip-study-plan.md#详细文档写作方法)，模块入口与资料索引按当前方法接续。
...
资料任务已于 2026-09-25 补读并修订，结果见[逐项补齐报告](source-reading-audit.md)。新增正式 CHI 选读、DFI 5.1 转录选读，补回 R13/C01 并加深关键笔记；访问受限与原版图表缺口仍明确保留。

## SWITCH 目录同步（2026-09-25）

研究正文、模型与资料已统一到 [SWITCH](SWITCH/README.md)。13 次历史更新依次重放后，以本地 v2.3 为最新内容；第 1–3 轮完成，后续从第 4 轮接续，详见[进度](SWITCH/RESEARCH_PROGRESS.md)与[迁移记录](switch-path-migration.md)。
