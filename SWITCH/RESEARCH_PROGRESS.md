...
资料集入口：[SWITCH 逐篇资料索引](../SWITCH/sources/README.md)，全局编号见[资料总入口](../sources.md)。模块上下文见 [SWITCH/README.md](../SWITCH/README.md)的“资料集与接续”。先根据索引导读选择相关笔记，复用其中的机制、版本与限制，再按需回原文；同一来源跨模块只维护一份主笔记。R9 是 CHI 模型指南、R12 是架构概述、R13 仅摘要已读，不能当完整规范或已核验的证明使用。本次资料整理不新增论文完成轮次。
