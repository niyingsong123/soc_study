...
2026-09-25 接续调整：用户要求先重整并审核既有中文详细稿，再继续后续轮次。通用规则已保存至[范本：详细文档写作方法](../chip-study-plan.md#详细文档写作方法)，具体接续见[当前重整方案](../SWITCH/research-plan.md#当前接续先重整已有详细稿)。当前仅保存方法和方案，正文重整尚未执行；上述第 1、2 轮完成及第 3–6 轮未完成的状态不变。可选深入问题不自动列为未完成任务，影响核心正确性的未知仍须核实。
...
执行重整时先读本页、[修订研究方案](../SWITCH/research-plan.md)、当前正文及相关资料笔记，重整后更新章节引用；随后接续第 3 轮时再核对旧第 49 章对应的后续安排及当前代码；从 GitHub 当前提交检查是否有并行修改。需要补充结论时区分公开来源、参考设计与目标芯片事实，新增细节同时审查是否改变 buffer、credit、ownership、ordering 或完成点。
