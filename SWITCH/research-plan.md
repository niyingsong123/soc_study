...
版本：v1.2；日期：2026-09-25；依据：[研究范本 v1.5](../chip-study-plan.md)。实际研究仍为第 1、2 轮完成，第 3–6 轮未完成。当前先重整并审核既有中文详细稿，之后再接续修订第 3 轮。本次仅保存通用方法与重整接续方案，尚未执行正文重整，不能记作第 3 轮成果。
...
采用[详细文档写作方法](../chip-study-plan.md#详细文档写作方法)，原位修订现有中文稿：先解释为什么需要 SWITCH，再讲典型功能/feature、整体微架构、子模块和接口，以完整事务例子串起 NI、Router 与适用的 D2D 边界，再按依赖细化核心机制、复杂行为和性能。保持一套明确的参考基线，不从公开资料猜测 AMD 目标实现。
...
完成时做技术及读者两种审核，更新图示、入口和本方案引用的章节定位；下表章节号目前仍指重整前正文。单纯叙述重排不重跑全部模型；实际改变容量、credit、ownership、ordering 或完成等契约时，再针对具体风险验证。此项完成后才按下表接续第 3 轮，当前未执行正文修改。
